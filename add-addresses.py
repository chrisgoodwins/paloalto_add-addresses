############################################################################
## This script can be used to create new address objects on a firewall or ##
## Panorama device group. The user will also have the option to add the   ##
## objects to an address group. The user has 2 options for adding the     ##
## address objects - either with a comma separated list, or by passing a  ##
## CSV file as a command line argument. The script tests for duplicates   ##
## within the list provided, as well as on the firewall or device group.  ##
############################################################################
#
#!/usr/bin/python
#
...
def getfwipfqdn():
    while True:
        try:
            fwipraw = raw_input("Please enter Panorama/firewall IP or FQDN: ")
...
def main():
    ...
    if devTree.find('./result/device-group/entry') == None:
        devType = 'fw'
